...
uint8_t nrf24_Transmit(nrf24 *node, uint8_t *data, uint8_t len) {
    ...
    spi_write_blocking(node->hSPIx,&cmdtosend,1);
	// HAL_SPI_Transmit(node->hSPIx, &cmdtosend, 1, NRF_TX_TIMEOUT);        cambio
    while(spi_is_busy(node->hSPIx) == 1)
	// while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);           cambio
	// send the payload
    if (len < 33) spi_write_blocking(node->hSPIx, data, len);
	//if (len < 33) HAL_SPI_Transmit(node->hSPIx, data, len, NRF_TX_TIMEOUT);     cambio
...
void nrf24_Receive(nrf24 *node, uint8_t *data, uint8_t len) {
    ...
    spi_write_blocking(node->hSPIx,&cmdtosend,1);
	// HAL_SPI_Transmit(node->hSPIx, &cmdtosend, 1, 100);             cambio 
    while(spi_is_busy(node->hSPIx) == 1)
	//while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);     cambio
	//Receive the payload
...
/* privated functions --------------------------------------------------------*/
static void nrf24_CE_enable(nrf24 *node) {
    gpio_put(node->CE_pin,1);
	//HAL_GPIO_WritePin(node->CE_port, node->CE_pin, GPIO_PIN_SET);     cambio
}
// -----------------------------------------------------------------------------

static void nrf24_CE_disable(nrf24 *node) {
    gpio_put(node->CE_pin,0);
	//HAL_GPIO_WritePin(node->CE_port, node->CE_pin, GPIO_PIN_RESET);       cambio
}
// -----------------------------------------------------------------------------

static void nrf24_CSN_enable(nrf24 *node) {
    gpio_put(node->CSN_pin,0);
	//HAL_GPIO_WritePin(node->CSN_port, node->CSN_pin, GPIO_PIN_RESET);     cambio
}
// -----------------------------------------------------------------------------

static void nrf24_CSN_disable(nrf24 *node) {
    gpio_put(node->CSN_pin,1);
	//HAL_GPIO_WritePin(node->CSN_port, node->CSN_pin, GPIO_PIN_SET);       cambio
}
// -----------------------------------------------------------------------------
...
static void nrf24_writeReg(nrf24 *node, uint8_t reg, uint8_t data) {
    ...
    spi_write_blocking(node->hSPIx, buf, 2);
	//HAL_SPI_Transmit(node->hSPIx, buf, 2, NRF_TX_TIMEOUT);    cambio
    while(spi_is_busy(node->hSPIx) == 1)
	//while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);     cambio
...
static void nrf24_writeRegMulti(nrf24 *node, uint8_t reg, uint8_t *data, uint16_t len) {
	...
	buf[0] = (reg | 1 << 5);
	nrf24_CSN_enable(node);
    spi_write_blocking(node->hSPIx, buf, 1);
	//HAL_SPI_Transmit(node->hSPIx, buf, 1, NRF_TX_TIMEOUT);        cambio
    while(spi_is_busy(node->hSPIx) == 1);
	//while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);     cambio
    spi_write_blocking(node->hSPIx, data, len);
	//HAL_SPI_Transmit(node->hSPIx, data, len, NRF_TX_TIMEOUT);     cambio
    while(spi_is_busy(node->hSPIx) == 1);
	//while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);     cambio
	nrf24_CSN_disable(node);
}
...
static uint8_t nrf24_readReg(nrf24 *node, uint8_t reg) {
	uint8_t data = 0;
	nrf24_CSN_enable(node);
    spi_write_blocking(node->hSPIx, &reg, 1);
...
static void nrf24_readRegMulti(nrf24 *node, uint8_t reg, uint8_t *data, uint16_t len) {
	...
	while(spi_is_busy(node->hSPIx) == 1);
	//while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);		cambio
	spi_read_blocking(node->hSPIx, 0, data, len);
	//HAL_SPI_Receive(node->hSPIx, data, len, NRF_RX_TIMEOUT);		cambio
	nrf24_CSN_disable(node);
}
// -----------------------------------------------------------------------------
...
static void nrf_sendCmd(nrf24 *node, uint8_t cmd) {
	nrf24_CSN_enable(node);
	spi_write_blocking(node->hSPIx, &cmd, 1);
	//HAL_SPI_Transmit(node->hSPIx, &cmd, 1, NRF_TX_TIMEOUT);		cambio
	while(spi_is_busy(node->hSPIx) == 1);
	//while (HAL_SPI_GetState(node->hSPIx) != HAL_SPI_STATE_READY);
	nrf24_CSN_disable(node);
}
